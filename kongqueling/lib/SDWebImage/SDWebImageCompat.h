...
NS_INLINE UIImage *SDScaledImageForPath(NSString *path, NSObject *imageOrData)
{
    ...
    // DISABLE scale while image == nil, initWithCGImage:scale:orientation: will return a UIImage even if we pass a nil image in.
    if ([[UIScreen mainScreen] respondsToSelector:@selector(scale)] && image)
    {
        CGFloat scale = 1.0;
        if (path.length >= 8)
        {
