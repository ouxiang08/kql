...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo
{
    ...
    {
        if ([delegate respondsToSelector:@selector(webImageManager:didFailAndStopTryingForURL:)])
        {
            [delegate performSelector:@selector(webImageManager:didFailAndStopTryingForURL:) withObject:self withObject: url];
        }
...
- (void)downloadWithURL:(NSURL *)url delegate:(id)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo success:(void (^)(UIImage *image))success failure:(void (^)(NSError *error))failure
{
    ...
    [cacheDelegates addObject:delegate];
    [cacheURLs addObject:url];
    SuccessBlock successCopy = success ? [success copy] : [^(UIImage* image){} copy];
    FailureBlock failureCopy = failure ? [failure copy] : [^(NSError *error){} copy];
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:
                          delegate, @"delegate",
                          url, @"url",
                          [NSNumber numberWithInt:options], @"options",
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    //Dream: image should be cache first
    if (image)
    {
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFailWithError:(NSError *)error;
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didFailAndStopTryingForURL:)])
            {
                [delegate performSelector:@selector(webImageManager:didFailAndStopTryingForURL:) withObject:self withObject: downloader.url];
            }
